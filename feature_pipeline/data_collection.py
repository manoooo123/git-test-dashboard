from __future__ import annotations

import logging
import os
import time
from dataclasses import dataclass
from pathlib import Path
from typing import Any

import pandas as pd
import requests
from dotenv import load_dotenv
from requests import Response
from requests.adapters import HTTPAdapter
from urllib3.util.retry import Retry


# ============================================================================
# PROJECT CONFIGURATION
# ============================================================================

PROJECT_ROOT = Path(__file__).resolve().parent.parent
ENV_FILE = PROJECT_ROOT / ".env"

RAW_DATA_DIR = PROJECT_ROOT / "data" / "raw"
PROCESSED_DATA_DIR = PROJECT_ROOT / "data" / "processed"

FINAL_DATASET_FILE = PROCESSED_DATA_DIR / "dataset_3cities.csv"

OPENAQ_URL = "https://api.openaq.org/v3"
OPEN_METEO_URL = (
    "https://archive-api.open-meteo.com/v1/archive"
)


# ============================================================================
# ENVIRONMENT
# ============================================================================

load_dotenv(ENV_FILE)

OPENAQ_API_KEY = os.getenv("OPENAQ_API_KEY")


# ============================================================================
# CONFIGURATION
# ============================================================================

REQUEST_TIMEOUT = 60
MAX_RETRIES = 4
BACKOFF_FACTOR = 1.5

OPENAQ_PAGE_SIZE = 1000
OPENAQ_LOCATION_RADIUS_METERS = 25_000

API_REQUEST_DELAY_SECONDS = 0.20

MERGE_TOLERANCE = pd.Timedelta("1 hour")


# ============================================================================
# CITIES
# ============================================================================

@dataclass(frozen=True)
class Location:
    """Target geographical location."""

    city: str
    latitude: float
    longitude: float


CITIES: tuple[Location, ...] = (
    Location(
        city="Lahore",
        latitude=31.5204,
        longitude=74.3587,
    ),
    Location(
        city="Islamabad",
        latitude=33.6844,
        longitude=73.0479,
    ),
    Location(
        city="Faisalabad",
        latitude=31.4504,
        longitude=73.1350,
    ),
)


# ============================================================================
# LOGGING
# ============================================================================

logging.basicConfig(
    level=logging.INFO,
    format=(
        "%(asctime)s | "
        "%(levelname)s | "
        "%(name)s | "
        "%(message)s"
    ),
)

logger = logging.getLogger(__name__)


# ============================================================================
# EXCEPTIONS
# ============================================================================

class DataIngestionError(RuntimeError):
    """Base ingestion error."""


class AuthenticationError(DataIngestionError):
    """Authentication failure."""


class APIRequestError(DataIngestionError):
    """API request failure."""


class DataValidationError(DataIngestionError):
    """Data validation failure."""


# ============================================================================
# CONFIGURATION VALIDATION
# ============================================================================

def validate_configuration() -> None:
    """Validate required environment variables."""

    if not OPENAQ_API_KEY:
        raise AuthenticationError(
            "OPENAQ_API_KEY is missing from .env"
        )

    if not OPENAQ_API_KEY.strip():
        raise AuthenticationError(
            "OPENAQ_API_KEY is empty."
        )

    logger.info("Runtime configuration validated.")


# ============================================================================
# HTTP SESSION
# ============================================================================

def create_session() -> requests.Session:
    """Create reusable HTTP session with retry support."""

    session = requests.Session()

    retry_policy = Retry(
        total=MAX_RETRIES,
        connect=MAX_RETRIES,
        read=MAX_RETRIES,
        status=MAX_RETRIES,
        backoff_factor=BACKOFF_FACTOR,
        status_forcelist=(
            429,
            500,
            502,
            503,
            504,
        ),
        allowed_methods=frozenset({"GET"}),
        raise_on_status=False,
    )

    adapter = HTTPAdapter(
        max_retries=retry_policy,
        pool_connections=10,
        pool_maxsize=10,
    )

    session.mount("https://", adapter)
    session.mount("http://", adapter)

    session.headers.update(
        {
            "Accept": "application/json",
            "User-Agent": (
                "Pearls-AQI-Predictor/1.0 "
                "(research-project)"
            ),
            "X-API-Key": OPENAQ_API_KEY,
        }
    )

    return session


# ============================================================================
# API REQUEST
# ============================================================================

def get_json(
    session: requests.Session,
    url: str,
    params: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Execute GET request and return JSON response."""

    try:
        response: Response = session.get(
            url,
            params=params,
            timeout=REQUEST_TIMEOUT,
        )

    except requests.RequestException as exc:
        raise APIRequestError(
            f"Network request failed: {exc}"
        ) from exc

    if response.status_code in (401, 403):
        raise AuthenticationError(
            "OpenAQ authentication failed. "
            "Check OPENAQ_API_KEY."
        )

    if response.status_code == 422:
        raise APIRequestError(
            "OpenAQ returned HTTP 422. "
            "Check query parameters."
        )

    if response.status_code >= 400:
        raise APIRequestError(
            f"API returned HTTP {response.status_code}."
        )

    try:
        payload = response.json()

    except ValueError as exc:
        raise APIRequestError(
            "API returned invalid JSON."
        ) from exc

    if not isinstance(payload, dict):
        raise APIRequestError(
            "Unexpected API response."
        )

    return payload


# ============================================================================
# LOCATION DISCOVERY
# ============================================================================

def find_locations(
    session: requests.Session,
    city: Location,
) -> pd.DataFrame:
    """Find real OpenAQ monitoring locations near a city."""

    logger.info(
        "[%s] Discovering OpenAQ monitoring locations.",
        city.city,
    )

    url = f"{OPENAQ_URL}/locations"

    params = {
        "coordinates": (
            f"{city.latitude},{city.longitude}"
        ),
        "radius": OPENAQ_LOCATION_RADIUS_METERS,
        "limit": 100,
    }

    payload = get_json(
        session,
        url,
        params,
    )

    results = payload.get("results", [])

    if not results:
        raise DataIngestionError(
            f"No OpenAQ locations found for {city.city}."
        )

    records: list[dict[str, Any]] = []

    for location in results:

        coordinates = location.get(
            "coordinates",
            {},
        )

        records.append(
            {
                "location_id": location.get("id"),
                "location_name": location.get("name"),
                "latitude": coordinates.get("latitude"),
                "longitude": coordinates.get("longitude"),
                "country": location.get("country"),
                "provider": location.get("provider"),
            }
        )

    locations = pd.DataFrame(records)

    locations.dropna(
        subset=["location_id"],
        inplace=True,
    )

    locations.drop_duplicates(
        subset=["location_id"],
        inplace=True,
    )

    locations.reset_index(
        drop=True,
        inplace=True,
    )

    logger.info(
        "[%s] Discovered %d OpenAQ locations.",
        city.city,
        len(locations),
    )

    return locations


# ============================================================================
# SENSOR DISCOVERY
# ============================================================================

def get_location_sensors(
    session: requests.Session,
    location_id: int,
) -> list[dict[str, Any]]:
    """Get sensors belonging to an OpenAQ location."""

    url = (
        f"{OPENAQ_URL}/locations/"
        f"{location_id}/sensors"
    )

    payload = get_json(
        session,
        url,
    )

    sensors = payload.get("results", [])

    if not isinstance(sensors, list):
        raise APIRequestError(
            f"Invalid sensor response for location "
            f"{location_id}."
        )

    return sensors


# ============================================================================
# PARAMETER NORMALIZATION
# ============================================================================

def normalize_parameter(
    parameter: str,
) -> str | None:
    """Normalize OpenAQ pollutant parameter names."""

    normalized = (
        str(parameter)
        .strip()
        .lower()
        .replace(".", "")
        .replace("_", "")
        .replace("-", "")
        .replace(" ", "")
    )

    if normalized in {
        "pm25",
        "particulatematter25",
    }:
        return "pm25"

    if normalized in {
        "pm10",
        "particulatematter10",
    }:
        return "pm10"

    return None


# ============================================================================
# SENSOR DATA
# ============================================================================

def collect_sensor_hours(
    session: requests.Session,
    sensor_id: int,
    parameter: str,
    start_date: str,
    end_date: str,
) -> pd.DataFrame:
    """Collect paginated hourly observations from OpenAQ."""

    url = (
        f"{OPENAQ_URL}/sensors/"
        f"{sensor_id}/hours"
    )

    records: list[dict[str, Any]] = []
    page = 1

    while True:

        params = {
            "datetime_from": (
                f"{start_date}T00:00:00Z"
            ),
            "datetime_to": (
                f"{end_date}T23:59:59Z"
            ),
            "limit": OPENAQ_PAGE_SIZE,
            "page": page,
        }

        payload = get_json(
            session,
            url,
            params,
        )

        results = payload.get(
            "results",
            [],
        )

        if not results:
            break

        for item in results:

            period = item.get(
                "period",
                {},
            )

            timestamp = (
                period
                .get("datetimeFrom", {})
                .get("utc")
            )

            value = item.get("value")

            if timestamp is None:
                continue

            if value is None:
                continue

            records.append(
                {
                    "timestamp": timestamp,
                    "sensor_id": sensor_id,
                    "parameter": parameter,
                    "value": value,
                }
            )

        logger.info(
            "Sensor=%s | Parameter=%s | "
            "Page=%s | Records=%s",
            sensor_id,
            parameter,
            page,
            len(results),
        )

        if len(results) < OPENAQ_PAGE_SIZE:
            break

        page += 1

        time.sleep(
            API_REQUEST_DELAY_SECONDS
        )

    if not records:
        return pd.DataFrame()

    data = pd.DataFrame(records)

    data["timestamp"] = pd.to_datetime(
        data["timestamp"],
        utc=True,
        errors="coerce",
    )

    data["value"] = pd.to_numeric(
        data["value"],
        errors="coerce",
    )

    data.dropna(
        subset=[
            "timestamp",
            "value",
        ],
        inplace=True,
    )

    data.drop_duplicates(
        subset=[
            "timestamp",
            "sensor_id",
            "parameter",
        ],
        inplace=True,
    )

    return data.reset_index(drop=True)


# ============================================================================
# LOCATION MEASUREMENTS
# ============================================================================

def collect_location_measurements(
    session: requests.Session,
    location_id: int,
    start_date: str,
    end_date: str,
) -> pd.DataFrame:
    """Collect PM2.5 and PM10 measurements."""

    sensors = get_location_sensors(
        session,
        location_id,
    )

    if not sensors:
        logger.warning(
            "Location %s has no sensors.",
            location_id,
        )
        return pd.DataFrame()

    frames: list[pd.DataFrame] = []

    for sensor in sensors:

        sensor_id = sensor.get("id")

        parameter_info = sensor.get(
            "parameter",
            {},
        )

        parameter_name = parameter_info.get(
            "name"
        )

        if sensor_id is None:
            continue

        if parameter_name is None:
            continue

        parameter = normalize_parameter(
            parameter_name
        )

        if parameter is None:
            continue

        logger.info(
            "Collecting %s | location=%s | sensor=%s",
            parameter,
            location_id,
            sensor_id,
        )

        try:

            data = collect_sensor_hours(
                session=session,
                sensor_id=int(sensor_id),
                parameter=parameter,
                start_date=start_date,
                end_date=end_date,
            )

        except DataIngestionError as exc:

            logger.warning(
                "Sensor %s failed: %s",
                sensor_id,
                exc,
            )

            continue

        if not data.empty:

            data["location_id"] = int(
                location_id
            )

            frames.append(data)

    if not frames:
        return pd.DataFrame()

    return pd.concat(
        frames,
        ignore_index=True,
    )


# ============================================================================
# AIR QUALITY NORMALIZATION
# ============================================================================

def normalize_air_quality(
    measurements: pd.DataFrame,
) -> pd.DataFrame:
    """Convert long OpenAQ observations to wide format."""

    if measurements.empty:
        raise DataValidationError(
            "Air-quality measurements are empty."
        )

    data = measurements.copy()

    required_columns = {
        "timestamp",
        "sensor_id",
        "location_id",
        "parameter",
        "value",
    }

    missing = (
        required_columns
        - set(data.columns)
    )

    if missing:
        raise DataValidationError(
            f"Missing OpenAQ columns: {sorted(missing)}"
        )

    data["timestamp"] = pd.to_datetime(
        data["timestamp"],
        utc=True,
        errors="coerce",
    )

    data["sensor_id"] = pd.to_numeric(
        data["sensor_id"],
        errors="coerce",
    )

    data["location_id"] = pd.to_numeric(
        data["location_id"],
        errors="coerce",
    )

    data["value"] = pd.to_numeric(
        data["value"],
        errors="coerce",
    )

    data["parameter"] = (
        data["parameter"]
        .astype(str)
        .str.lower()
        .str.strip()
    )

    data.dropna(
        subset=[
            "timestamp",
            "sensor_id",
            "location_id",
            "parameter",
            "value",
        ],
        inplace=True,
    )

    wide = (
        data
        .pivot_table(
            index=[
                "location_id",
                "sensor_id",
                "timestamp",
            ],
            columns="parameter",
            values="value",
            aggfunc="mean",
        )
        .reset_index()
    )

    wide.columns.name = None

    wide.rename(
        columns={
            "pm25": "pm2_5",
        },
        inplace=True,
    )

    if "pm2_5" not in wide.columns:
        raise DataValidationError(
            "PM2.5 was not found."
        )

    logger.info(
        "Normalized OpenAQ observations: %d",
        len(wide),
    )

    logger.info(
        "Unique monitoring sensors: %d",
        wide["sensor_id"].nunique(),
    )

    logger.info(
        "Unique monitoring locations: %d",
        wide["location_id"].nunique(),
    )

    return (
        wide
        .sort_values(
            [
                "timestamp",
                "location_id",
                "sensor_id",
            ],
            kind="mergesort",
        )
        .reset_index(drop=True)
    )


# ============================================================================
# WEATHER
# ============================================================================

def collect_weather(
    session: requests.Session,
    city: Location,
    start_date: str,
    end_date: str,
) -> pd.DataFrame:
    """Collect historical hourly weather for a city."""

    logger.info(
        "[%s] Collecting historical weather.",
        city.city,
    )

    params = {
        "latitude": city.latitude,
        "longitude": city.longitude,
        "start_date": start_date,
        "end_date": end_date,
        "hourly": (
            "temperature_2m,"
            "relative_humidity_2m,"
            "surface_pressure,"
            "cloud_cover,"
            "wind_speed_10m,"
            "wind_direction_10m,"
            "precipitation"
        ),
        "timezone": "Asia/Karachi",
    }

    try:

        response = requests.get(
            OPEN_METEO_URL,
            params=params,
            timeout=(15, 180),
        )

    except requests.RequestException as exc:

        raise APIRequestError(
            f"Open-Meteo request failed for "
            f"{city.city}: {exc}"
        ) from exc

    if response.status_code >= 400:

        raise APIRequestError(
            f"Open-Meteo returned HTTP "
            f"{response.status_code} for {city.city}: {response.text}"
        )

    payload = response.json()

    hourly = payload.get("hourly")

    if not isinstance(hourly, dict):
        raise DataValidationError(
            f"Invalid weather response for {city.city}."
        )

    weather = pd.DataFrame(hourly)

    if weather.empty:
        raise DataValidationError(
            f"No weather data for {city.city}."
        )

    weather.rename(
        columns={
            "time": "timestamp",
            "temperature_2m": "temperature",
            "relative_humidity_2m": "humidity",
            "surface_pressure": "pressure",
            "cloud_cover": "clouds",
            "wind_speed_10m": "wind_speed",
            "wind_direction_10m": "wind_direction",
        },
        inplace=True,
    )

    weather["timestamp"] = pd.to_datetime(
        weather["timestamp"],
        errors="coerce",
    )

    weather.dropna(
        subset=["timestamp"],
        inplace=True,
    )

    weather["timestamp"] = (
        weather["timestamp"]
        .dt.tz_localize("Asia/Karachi")
        .dt.tz_convert("UTC")
    )

    numeric_columns = [
        "temperature",
        "humidity",
        "pressure",
        "clouds",
        "wind_speed",
        "wind_direction",
        "precipitation",
    ]

    for column in numeric_columns:

        if column in weather.columns:

            weather[column] = pd.to_numeric(
                weather[column],
                errors="coerce",
            )

    weather["city"] = city.city

    weather.drop_duplicates(
        subset=["timestamp"],
        inplace=True,
    )

    weather.sort_values(
        "timestamp",
        inplace=True,
    )

    weather.reset_index(
        drop=True,
        inplace=True,
    )

    logger.info(
        "[%s] Weather observations: %d",
        city.city,
        len(weather),
    )

    return weather


# ============================================================================
# DATASET MERGE
# ============================================================================

def build_dataset(
    air_quality: pd.DataFrame,
    weather: pd.DataFrame,
    city: Location,
) -> pd.DataFrame:
    """Merge air-quality and weather data."""

    if air_quality.empty:
        raise DataValidationError(
            "Air-quality dataset is empty."
        )

    if weather.empty:
        raise DataValidationError(
            "Weather dataset is empty."
        )

    air = air_quality.copy()
    met = weather.copy()

    air["timestamp"] = pd.to_datetime(
        air["timestamp"],
        utc=True,
        errors="coerce",
    )

    met["timestamp"] = pd.to_datetime(
        met["timestamp"],
        utc=True,
        errors="coerce",
    )

    air.dropna(
        subset=["timestamp"],
        inplace=True,
    )

    met.dropna(
        subset=["timestamp"],
        inplace=True,
    )

    if "location_id" not in air.columns:
        raise DataValidationError(
            "location_id missing from air-quality data."
        )

    if "pm2_5" not in air.columns:
        raise DataValidationError(
            "PM2.5 column is missing."
        )

    air.drop_duplicates(
        subset=[
            "location_id",
            "sensor_id",
            "timestamp",
        ],
        keep="last",
        inplace=True,
    )

    # merge_asof requires timestamp to be globally sorted.
    air.sort_values(
        [
            "timestamp",
            "location_id",
            "sensor_id",
        ],
        kind="mergesort",
        inplace=True,
    )

    met.sort_values(
        ["timestamp"],
        kind="mergesort",
        inplace=True,
    )

    air.reset_index(drop=True, inplace=True)
    met.reset_index(drop=True, inplace=True)

    if not air["timestamp"].is_monotonic_increasing:
        raise DataValidationError(
            "Air-quality timestamps are not sorted."
        )

    if not met["timestamp"].is_monotonic_increasing:
        raise DataValidationError(
            "Weather timestamps are not sorted."
        )

    dataset = pd.merge_asof(
        air,
        met.drop(columns=["city"], errors="ignore"),
        on="timestamp",
        direction="nearest",
        tolerance=MERGE_TOLERANCE,
    )

    dataset["city"] = city.city

    dataset.dropna(
        subset=["pm2_5"],
        inplace=True,
    )

    if dataset.empty:
        raise DataValidationError(
            f"No valid observations after merge "
            f"for {city.city}."
        )

    dataset.sort_values(
        [
            "location_id",
            "sensor_id",
            "timestamp",
        ],
        kind="mergesort",
        inplace=True,
    )

    dataset.reset_index(
        drop=True,
        inplace=True,
    )

    logger.info(
        "[%s] Final merged rows: %d",
        city.city,
        len(dataset),
    )

    logger.info(
        "[%s] Locations: %d | Sensors: %d",
        city.city,
        dataset["location_id"].nunique(),
        dataset["sensor_id"].nunique(),
    )

    return dataset


# ============================================================================
# VALIDATION
# ============================================================================

def validate_dataset(
    dataset: pd.DataFrame,
    city: str,
) -> None:
    """Validate city-level final dataset."""

    if dataset.empty:
        raise DataValidationError(
            f"{city}: final dataset is empty."
        )

    required_columns = {
        "timestamp",
        "city",
        "location_id",
        "sensor_id",
        "pm2_5",
    }

    missing = (
        required_columns
        - set(dataset.columns)
    )

    if missing:
        raise DataValidationError(
            f"{city}: missing columns {sorted(missing)}"
        )

    if dataset["timestamp"].isna().any():
        raise DataValidationError(
            f"{city}: null timestamps remain."
        )

    if dataset["pm2_5"].isna().all():
        raise DataValidationError(
            f"{city}: all PM2.5 values are missing."
        )

    if (dataset["pm2_5"] < 0).any():
        raise DataValidationError(
            f"{city}: negative PM2.5 values detected."
        )

    logger.info(
        "[%s] Dataset validation PASSED.",
        city,
    )


# ============================================================================
# QUALITY REPORT
# ============================================================================

def generate_quality_report(
    dataset: pd.DataFrame,
    city: str,
) -> None:
    """Generate city-level data quality report."""

    logger.info("=" * 72)
    logger.info("DATA QUALITY REPORT | %s", city)
    logger.info("=" * 72)

    logger.info(
        "Rows: %d",
        len(dataset),
    )

    logger.info(
        "Columns: %d",
        len(dataset.columns),
    )

    logger.info(
        "Locations: %d",
        dataset["location_id"].nunique(),
    )

    logger.info(
        "Sensors: %d",
        dataset["sensor_id"].nunique(),
    )

    logger.info(
        "Start: %s",
        dataset["timestamp"].min(),
    )

    logger.info(
        "End: %s",
        dataset["timestamp"].max(),
    )

    if "temperature" in dataset.columns:

        weather_match = (
            dataset["temperature"]
            .notna()
            .mean()
            * 100
        )

        logger.info(
            "Weather matched: %.2f%%",
            weather_match,
        )

    logger.info(
        "Missing values:\n%s",
        dataset.isna().sum().to_string(),
    )

    logger.info("=" * 72)


# ============================================================================
# SAVE
# ============================================================================

def save_csv(
    dataframe: pd.DataFrame,
    destination: Path,
) -> None:
    """Atomically save a DataFrame as CSV."""

    destination.parent.mkdir(
        parents=True,
        exist_ok=True,
    )

    temporary_file = destination.with_suffix(
        ".tmp.csv"
    )

    dataframe.to_csv(
        temporary_file,
        index=False,
    )

    temporary_file.replace(
        destination
    )

    logger.info(
        "Saved: %s",
        destination,
    )


# ============================================================================
# CITY PIPELINE
# ============================================================================

def run_city_ingestion(
    session: requests.Session,
    city: Location,
    start_date: str,
    end_date: str,
) -> pd.DataFrame:
    """Run complete real-data ingestion for one city."""

    logger.info("")
    logger.info("#" * 72)
    logger.info(
        "STARTING CITY: %s",
        city.city.upper(),
    )
    logger.info(
        "Historical period: %s → %s",
        start_date,
        end_date,
    )
    logger.info("#" * 72)

    city_slug = city.city.lower()

    air_file = (
        RAW_DATA_DIR
        / f"openaq_{city_slug}.csv"
    )

    weather_file = (
        RAW_DATA_DIR
        / f"weather_{city_slug}.csv"
    )

    dataset_file = (
        PROCESSED_DATA_DIR
        / f"dataset_{city_slug}.csv"
    )

    # ---------------------------------------------------------------
    # LOCATIONS
    # ---------------------------------------------------------------

    locations = find_locations(
        session,
        city,
    )

    # ---------------------------------------------------------------
    # AIR QUALITY
    # ---------------------------------------------------------------

    measurement_frames: list[pd.DataFrame] = []

    for location_id in locations[
        "location_id"
    ].dropna():

        try:

            location_data = (
                collect_location_measurements(
                    session=session,
                    location_id=int(location_id),
                    start_date=start_date,
                    end_date=end_date,
                )
            )

            if not location_data.empty:
                measurement_frames.append(
                    location_data
                )

        except DataIngestionError as exc:

            logger.warning(
                "[%s] Location %s skipped: %s",
                city.city,
                location_id,
                exc,
            )

    if not measurement_frames:

        raise DataIngestionError(
            f"No real OpenAQ measurements "
            f"collected for {city.city}."
        )

    raw_air_quality = pd.concat(
        measurement_frames,
        ignore_index=True,
    )

    raw_air_quality.drop_duplicates(
        inplace=True
    )

    save_csv(
        raw_air_quality,
        air_file,
    )

    # ---------------------------------------------------------------
    # NORMALIZE
    # ---------------------------------------------------------------

    air_quality = normalize_air_quality(
        raw_air_quality
    )

    # ---------------------------------------------------------------
    # WEATHER
    # ---------------------------------------------------------------

    weather = collect_weather(
        session=session,
        city=city,
        start_date=start_date,
        end_date=end_date,
    )

    save_csv(
        weather,
        weather_file,
    )

    # ---------------------------------------------------------------
    # MERGE
    # ---------------------------------------------------------------

    dataset = build_dataset(
        air_quality=air_quality,
        weather=weather,
        city=city,
    )

    # ---------------------------------------------------------------
    # VALIDATE
    # ---------------------------------------------------------------

    validate_dataset(
        dataset,
        city.city,
    )

    generate_quality_report(
        dataset,
        city.city,
    )

    save_csv(
        dataset,
        dataset_file,
    )

    logger.info(
        "CITY COMPLETED SUCCESSFULLY: %s",
        city.city,
    )

    return dataset


# ============================================================================
# COMPLETE 3-CITY INGESTION
# ============================================================================

def run_ingestion(
    start_date: str,
    end_date: str,
) -> pd.DataFrame:
    """Run real-data ingestion for all configured cities."""

    validate_configuration()

    RAW_DATA_DIR.mkdir(
        parents=True,
        exist_ok=True,
    )

    PROCESSED_DATA_DIR.mkdir(
        parents=True,
        exist_ok=True,
    )

    session = create_session()

    city_datasets: list[pd.DataFrame] = []

    failures: list[str] = []

    for city in CITIES:

        try:

            dataset = run_city_ingestion(
                session=session,
                city=city,
                start_date=start_date,
                end_date=end_date,
            )

            city_datasets.append(dataset)

        except DataIngestionError as exc:

            failures.append(
                f"{city.city}: {exc}"
            )

            logger.exception(
                "CITY FAILED: %s",
                city.city,
            )

    if not city_datasets:

        raise DataIngestionError(
            "No city completed successfully."
        )

    final_dataset = pd.concat(
        city_datasets,
        ignore_index=True,
    )

    final_dataset.sort_values(
        [
            "city",
            "location_id",
            "sensor_id",
            "timestamp",
        ],
        kind="mergesort",
        inplace=True,
    )

    final_dataset.reset_index(
        drop=True,
        inplace=True,
    )

    # ---------------------------------------------------------------
    # FINAL GLOBAL VALIDATION
    # ---------------------------------------------------------------

    required_cities = {
        "Lahore",
        "Islamabad",
        "Faisalabad",
    }

    available_cities = set(
        final_dataset["city"].unique()
    )

    missing_cities = (
        required_cities
        - available_cities
    )

    if missing_cities:

        raise DataValidationError(
            "Required cities missing from final "
            f"dataset: {sorted(missing_cities)}"
        )

    save_csv(
        final_dataset,
        FINAL_DATASET_FILE,
    )

    # ---------------------------------------------------------------
    # FINAL REPORT
    # ---------------------------------------------------------------

    logger.info("")
    logger.info("=" * 72)
    logger.info(
        "FINAL 3-CITY DATA QUALITY REPORT"
    )
    logger.info("=" * 72)

    logger.info(
        "Total rows: %d",
        len(final_dataset),
    )

    logger.info(
        "Total columns: %d",
        len(final_dataset.columns),
    )

    logger.info(
        "Cities: %s",
        sorted(
            final_dataset["city"]
            .unique()
            .tolist()
        ),
    )

    logger.info(
        "Rows by city:\n%s",
        final_dataset["city"]
        .value_counts()
        .sort_index()
        .to_string(),
    )

    logger.info(
        "Total monitoring locations: %d",
        final_dataset["location_id"].nunique(),
    )

    logger.info(
        "Total monitoring sensors: %d",
        final_dataset["sensor_id"].nunique(),
    )

    logger.info(
        "Global start: %s",
        final_dataset["timestamp"].min(),
    )

    logger.info(
        "Global end: %s",
        final_dataset["timestamp"].max(),
    )

    logger.info("=" * 72)

    if failures:

        logger.warning(
            "Some city pipelines failed:"
        )

        for failure in failures:
            logger.warning(
                "  - %s",
                failure,
            )

        raise DataIngestionError(
            "Pipeline completed partially. "
            "Check logs for failed cities."
        )

    logger.info(
        "ALL 3 CITIES COMPLETED SUCCESSFULLY."
    )

    return final_dataset


# ============================================================================
# MAIN
# ============================================================================

def main() -> None:
    """Application entry point."""

    dataset = run_ingestion(
        start_date="2023-01-01",
        end_date="2026-08-22",
    )

    print("\n" + "=" * 72)

    print(
        "PEARLS AQI PREDICTOR"
    )

    print(
        "3-CITY REAL DATA INGESTION SUCCESS"
    )

    print("=" * 72)

    print(
        "\nDataset shape:"
    )

    print(
        dataset.shape
    )

    print(
        "\nCities:"
    )

    print(
        dataset["city"]
        .value_counts()
        .sort_index()
    )

    print(
        "\nColumns:"
    )

    print(
        dataset.columns.tolist()
    )

    print(
        "\nFirst 10 rows:"
    )

    print(
        dataset.head(10)
        .to_string(index=False)
    )

    print(
        "\nFinal dataset:"
    )

    print(
        FINAL_DATASET_FILE
    )


if __name__ == "__main__":
    main()

